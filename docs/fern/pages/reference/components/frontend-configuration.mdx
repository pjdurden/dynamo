...
<ParamField path="--router-session-affinity-mode" type="string" default="hard">
  `hard` routes a bound session directly to its worker. `soft` requires `--router-mode kv`; it prefers the bound worker while still running worker filters, scorers, and pickers.

  <span className="enum-values"><span className="enum-label">Allowed values:</span> <Badge intent="note" minimal>hard</Badge> <Badge intent="note" minimal>soft</Badge></span>
